...
from nlp.conversation import process_conversation, conversation_manager
import numpy as np
...
def chat_with_ai(user_message: str):
    """
    사용자 메시지에 대한 단계별 대화 응답을 생성합니다.
    구체적인 정보가 필요한 경우 추가 질문을 통해 맥락을 수집합니다.
    """
    
    # 대화 처리
    contextual_response, is_final = process_conversation(user_message)
    ...
    if contextual_response:
        # 구체적인 검색 쿼리가 생성된 경우
        search_query = contextual_response
    else:
        # 일반 질문인 경우
